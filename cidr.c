...
void process_ip(u32 saddr){
	if(ip_in_range(saddr, &aws_cidr_blocks[0][0], aws_cidr_arr_rows, aws_cidr_arr_cols))
		increment_counter(AWS_CLOUD);
	else if(ip_in_range(saddr, &gcp_cidr_blocks[0][0], gcp_cidr_arr_rows, gcp_cidr_arr_cols))
		increment_counter(GCP_CLOUD);
	else if(ip_in_range(saddr, &azure_cidr_blocks[0][0], azure_cidr_arr_rows, azure_cidr_arr_cols))
